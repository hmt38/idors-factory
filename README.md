...
- **IDOR Detection & Exploitation** (New!)
...
## IDOR Detection (New Feature)

Autorize now includes a dedicated module for detecting Insecure Direct Object References (IDOR).

### Workflow
1. **Capture Traffic**: Browse the application as two different users (User A and User B). Autorize automatically captures and analyzes traffic to identify potential IDOR parameters (e.g., IDs, UUIDs).
2. **Generate Attacks**: Go to the **IDOR Attacks** tab and click **Generate Attacks**. The plugin will:
   - Identify sensitive parameters in User A's requests.
   - Find matching parameters from User B's history.
   - Generate attack payloads by swapping User A's parameters with User B's values.
3. **Review & Execute**:
   - Review the generated attack list.
   - **Sensitive APIs** (POST/PUT/DELETE) are highlighted in **RED** and require manual execution.
   - Click **Execute Selected** to run the attack. The plugin will replay the request with the modified parameter while preserving User A's session (Cookies/Headers).
4. **Analysis**:
   - The plugin checks the response status code.
   - If enabled, it uses an **LLM (Large Language Model)** to intelligently analyze the response and determine if the attack was successful (Vulnerable) or safe.
