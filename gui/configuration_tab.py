                ...
                if hasattr(self._extender, "progressBar"):
                    ...
                    def reset():
                        ...
                        if hasattr(self._extender, "progressBar"):
                            ...
                            SwingUtilities.invokeLater(
                                lambda: self._extender.progressBar.setStringPainted(
                                    False
                                )
                            )
    ...
    def run(self):
        try:
            ...
            if hasattr(self._extender, "progressBar"):
                ...
                SwingUtilities.invokeLater(
                    lambda: self._extender.progressBar.setString("Testing LLM...")
                )
                SwingUtilities.invokeLater(
                    lambda: self._extender.progressBar.setStringPainted(True)
                )
            ...
            if hasattr(self._extender, "progressBar"):
                ...
                SwingUtilities.invokeLater(
                    lambda: self._extender.progressBar.setString(msg)
                )
                ...
                def reset():
                    ...
                    if hasattr(self._extender, "progressBar"):
                        ...
                        SwingUtilities.invokeLater(
                            lambda: self._extender.progressBar.setStringPainted(False)
                        )
...
class ConfigurationTab:
    ...
    def draw(self):
        ...
        self._extender.startButton = JToggleButton(
            "Autorize is off", actionPerformed=self.startOrStop
        )
        self._extender.startButton.setBounds(10, 20, 230, 30)

        self._extender.clearButton = JButton(
            "Clear table", actionPerformed=self.clearTable
        )
        self._extender.clearButton.setBounds(10, 80, 100, 30)

        self._extender.clearDbButton = JButton(
            "Clear DB", actionPerformed=self.clearDatabase
        )
        self._extender.clearDbButton.setBounds(10, 120, 100, 30)
        self._extender.clearDbButton.setToolTipText(
            "Clear all data from the local database (requests, parameters, attacks)"
        )

        self._extender.runExtractorButton = JButton(
            "Extract Params", actionPerformed=self.runExtractor
        )
        self._extender.runExtractorButton.setBounds(10, 160, 120, 30)
        self._extender.runExtractorButton.setToolTipText(
            "Manually extract parameters from captured requests"
        )

        self._extender.generateAttacksButton = JButton(
            "Generate Attacks", actionPerformed=self.generateAttacks
        )
        self._extender.generateAttacksButton.setBounds(10, 200, 140, 30)
        self._extender.generateAttacksButton.setToolTipText(
        ...
        self._extender.interceptRequestsfromRepeater = JCheckBox(
        ...
        self._extender.replaceQueryParam = JCheckBox(
            "Replace query params", actionPerformed=self.replaceQueryHanlder
        )
        ...
        self._extender.filtersTabs.addTab(
            "Unauthentication Detector ", self._extender.EDPnlUnauth
        )
        self._extender.filtersTabs.addTab(
            "Interception Filters", self._extender.filtersPnl
        )
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(
                ...
                .addGroup(
                    ...
                    .addComponent(
                        self._extender.llmModel,
                        GroupLayout.PREFERRED_SIZE,
                        GroupLayout.PREFERRED_SIZE,
                        GroupLayout.PREFERRED_SIZE,
                    )
                    .addComponent(
                        self._extender.testLlmButton,
                        GroupLayout.PREFERRED_SIZE,
                        GroupLayout.PREFERRED_SIZE,
                        GroupLayout.PREFERRED_SIZE,
                    )
        ...
        if hasattr(self._extender, "userTab") and self._extender.userTab:
            for user_id, user_data in self._extender.userTab.user_tabs.items():
                if self._extender.replaceQueryParam.isSelected():
                    user_data["headers_instance"].replaceString.setText(
